...
public class VoxelChunk : MonoBehaviour
{
    ...
    int GetTerrainHeight(int worldX, int worldZ)
    {
        ...
        for (int i = 0; i < octaves; i++)
        {
            ...
            noiseHeight += perlinValue * amplitude;  //진폭을 적용해서 누적함.

            amplitude *= persistence;  //다음 옥타브로 갈 수록 진폭을 줄인다.
            frequency *= persistence;  //주파수를 조정.
        }

        //높이 범위 조정
        int height = groundLevel + Mathf.RoundToInt(noiseHeight * heightVariation);  //기본 지면 높이 적용
        return Mathf.Clamp(height, 1, chunkHeight - 1);  //높이릉 최소 1에서 최대 사이로 제한.
    }
