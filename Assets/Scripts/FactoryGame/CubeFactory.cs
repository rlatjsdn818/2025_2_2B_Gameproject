...
public class CubeFactory : MonoBehaviour
{
    ...
    void ProcessRequests()
    {
        ...
        if (products[firstRequest.productType] >= firstRequest.quantity)
        {
            ...
            requestList.RemoveAt(0);    //리스트서 첫번째 제거
        }
        else
        {
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(10, 40, 250, 20), $"나무 창고 (Stack) : {woodWarehouse.Count} 개");
        GUI.Label(new Rect(10, 40, 250, 20), $"금속 창고 (Stack) : {metalWarehouse.Count} 개");
        GUI.Label(new Rect(10, 40, 250, 20), $"조립 스택 (Stack) : {assemblyStack.Count} 개 작업");
        GUI.Label(new Rect(10, 40, 250, 20), $"완제품 (Dict) : {products[ProductType.Chair]} 개");
        GUI.Label(new Rect(10, 40, 250, 20), $"요청서 (List) : {requestList.Count} 개");
